...
geo_inp = []
for f in files:
    geo_inp += [read_geo(os.path.join('partitioned', 'solid_inp_' + str(f).zfill(3) + '.vtu')).GetOutput()]

geo_out = []
for f in files:
    geo_out += [read_geo(os.path.join('partitioned', 'solid_out_' + str(f).zfill(3) + '.vtu')).GetOutput()]
